...
| 组件 | 版本要求 | 用途 |
|------|----------|------|
| Node.js | 16.x 或以上 | 前端构建与开发 |
| Python | 3.8 或以上 | 后端服务 |
| MySQL | 5.7 或以上 | 业务数据存储 |
| Java | JDK 8 或以上 | WeBASE-Front 区块链浏览器 |
| FISCO BCOS | 3.0+ | 联盟链底层 |
...
该脚本依次启动 MySQL → FISCO BCOS 4节点 → FastAPI 后端 → Vue 前端 → WeBASE-Front 区块链浏览器，并在最后展示所有端口占用状态。
...
项目额外部署了 FISCO BCOS 官方区块链浏览器 WeBASE-Front v3.1.1，独立于项目运行：

| 项目 | 值 |
|------|------|
| 访问地址 | http://localhost:5002/WeBASE-Front |
| 部署位置 | `~/webase-front/`（项目目录外） |
| 连接节点 | node0 RPC 127.0.0.1:20200 |
| 功能 | 区块浏览、交易详情、合约管理、节点监控 |
...
| 扫码识别 | 支持摄像头实时扫码和图片上传识别，自动校验溯源码格式 |
...
|------|------|------|
| MySQL | 3306 | 数据库 |
| FISCO node0-3 RPC | 20200-20203 | 区块链节点 RPC 接口 |
| FISCO node0-3 P2P | 30300-30303 | 区块链节点间通信 |
| FastAPI 后端 | 8000 | 后端 API 服务 |
| Vue 前端 | 5173 | 前端开发服务器 |
| WeBASE-Front | 5002 | 区块链浏览器 |
